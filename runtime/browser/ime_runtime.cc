...
#include "runtime/browser/native_ime_app_window.h"
...
ImeRuntime::ImeRuntime(std::unique_ptr<common::ApplicationData> app_data)
    : application_(NULL),
      native_window_(NULL),
...
void ImeRuntime::OnCreate() {
  ...
  application_ = new ImeApplication(native_window_, std::move(app_data_));
  ...
  Evas_Object *ime_window = ime_get_main_window();
  if (!ime_window) {
...
void ImeRuntime::OnShow(int context_id, ime_context_h context) {
  ...
  Evas_Object *ime_window = ime_get_main_window();
  if (!ime_window) {
    LOGGER(DEBUG) << get_last_result();
    return;
  }

  evas_object_show(ime_window);
}
...
void ime_app_main(int argc, char **argv) {
  ...
  ops.create = [](void* data) -> void {
    runtime::ImeRuntime* runtime = reinterpret_cast<runtime::ImeRuntime*>(data);
    if (!runtime) {
  ...
  ops.show = [](int context_id, ime_context_h context, void *data) -> void {
    ...
    runtime->OnShow(context_id, context);
  };
...
int ImeRuntime::Exec(int argc, char* argv[]) {
  ...
  ime_event_set_cursor_position_updated_cb(ime_app_cursor_position_updated_cb, this);
  ime_event_set_layout_set_cb(ime_app_layout_set_cb, this);
  ime_event_set_return_key_type_set_cb(ime_app_return_key_type_set_cb, this);
  ime_event_set_return_key_state_set_cb(ime_app_return_key_state_set_cb, this);
  ime_event_set_process_key_event_cb(ime_app_process_key_event_cb, this);
  ime_event_set_display_language_changed_cb(ime_app_display_language_changed_cb, this);
